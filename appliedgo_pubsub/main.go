/* all rights reserved by Christoph Berger
https://github.com/AppliedGo/messaging/blob/master/messaging.go */
package main
...
func runServer(url string, topics []string) {
	...
	for i := 0; i < 5; i++ {
		for _, topic := range topics {
			...
			err = publish(socket, topic, fmt.Sprintf("topic = %+v\n", topic))
			if err != nil {
...
func runClient(name, url string, topics []string) {
	...
	if err != nil {
		log.Fatalf("Cannot dial int %s: %s\n", url, err.Error())
	}
	for _, topic := range topics {
